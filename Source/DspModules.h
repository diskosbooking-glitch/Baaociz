...
//  NOISE RISER
//
//  Remplace la sinusoide harmonique. Deux bandes de bruit tres resonantes
//  suivent la meme trajectoire que le potard, plus un lit de bruit large qui
//  s'ouvre progressivement.
//
//  L'interet : a facteur de qualite eleve, une bande de bruit laisse percevoir
//  une hauteur qui monte, mais sans tonalite propre. Elle ne peut donc pas se
//  disputer avec la tonalite du morceau, contrairement a une sinusoide calee
//  sur un La.
//
//  La sortie passe-bande de JUCE a un gain au centre egal au facteur de
//  qualite ; on divise par sa racine pour retrouver un gain en racine de Q,
//  qui compense la bande passante qui se resserre sans exploser le niveau.
//==============================================================================
class NoiseRiser
{
public:
    ...
    {
        sr = spec.sampleRate;
        for (auto* f : { &band1, &band2, &bed })
        {
            f->prepare (spec);
            f->setType (juce::dsp::StateVariableTPTFilterType::bandpass);
            f->reset();
        }
        band1.setResonance (4.0f);
        band2.setResonance (4.0f);
        bed.setResonance (0.7f);
        invSqrtQ = 0.5f;
    }

    void reset() { band1.reset(); band2.reset(); bed.reset(); }

    void update (double f0, float q, float intensity)
    {
        const double nyq = sr * 0.45;
        q = juce::jlimit (1.0f, 40.0f, q);

        band1.setResonance (q);
        band2.setResonance (q);
        band1.setCutoffFrequency ((float) juce::jlimit (40.0, nyq, f0 * 2.0));
        band2.setCutoffFrequency ((float) juce::jlimit (40.0, nyq, f0 * 3.0));
        bed.setCutoffFrequency ((float) juce::jlimit (100.0, nyq,
                                 400.0 + 3000.0 * (double) intensity));

        invSqrtQ = 1.0f / std::sqrt (q);
    }

    inline float process (int channel, float noise, float bandGain, float bedGain) noexcept
    {
        const float b1 = band1.processSample (channel, noise);
        const float b2 = band2.processSample (channel, noise);
        const float bd = bed.processSample   (channel, noise);
        return (b1 + 0.5f * b2) * invSqrtQ * bandGain + bd * bedGain;
    }

    void snapToZero() noexcept { band1.snapToZero(); band2.snapToZero(); bed.snapToZero(); }

private:
    juce::dsp::StateVariableTPTFilter<float> band1, band2, bed;
    double sr = 44100.0;
    float  invSqrtQ = 0.5f;
};
