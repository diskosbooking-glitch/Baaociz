...
void SkygrinAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer&)
{
    ...
    while (pos < numSamples)
    {
        ...
        //  le bruit resonant suit la meme trajectoire que le riser.
        //  La resonance maximale depend du preset : douce et souflee sur les
        //  presets calmes, quasi tonale sur les presets durs.
        const float noiseBand = cur[ModNoise] * 0.30f;
        const float noiseBed  = cur[ModNoise] * 0.13f;
        const bool  useNoise  = cur[ModNoise] > 0.002f;
        noiseRiser.update (riser.getFundamental(), 2.0f + cur[ModRes] * 26.0f, t);
        ...
        for (int s = 0; s < n; ++s)
        {
            ...
            for (int ch = 0; ch < numCh; ++ch)
            {
                ...
                if (useNoise)
                {
                    ...
                    x += noiseRiser.process (ch, nz, noiseBand, noiseBed);
                }
